...
import {
  GremioConsolidadorAPI,
  GremioConsolidadorDetalleResponse,
} from "@/types/microMaestros/gremiosConsolidadoresTypes"
import { fetchGremiosConsolidadores } from "@/services/microMaestros/consolidadorService"

interface InputsCreateUpdateProps {
  formik: any;
  response?: GremioConsolidadorDetalleResponse | null;
}

export function InputsCreateUpdate({ formik, response }: InputsCreateUpdateProps) {
  const [gremiosExisting, setGremiosExisting] = useState<GremioConsolidadorAPI[]>([])

  const handleChangeGremio = (event: any) => {
    formik.handleChange(event)
    if (event.target.value.length > 2) {
      fetchGremiosConsolidadores({ nombre: event.target.value })
        .then((response) => {
