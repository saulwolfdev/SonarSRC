...
import { fetchDiagramaTrabajoById, putDiagramaTrabajo } from './services/DiagramaTrabajoService';
import { DiagramaTrabajoDetalleResponse } from './types/diagramaTrabajoTypes';
import Spinner from '@/components/shared/Spinner';

export const EditDiagramaTrabajo: React.FC = () => {
    ...
    useEffect(() => {
        if (numericId !== undefined) {
            fetchDiagramaTrabajoById(numericId)
                .then((data: DiagramaTrabajoDetalleResponse) => {
                    console.log(data)
                    formik.setValues({
                        nombre: data.nombre,
                        cuit: data.cuit
        ...
        onSubmit: async (values) => {
            if (numericId !== undefined) {
                try {
                    ...
                    await putDiagramaTrabajo({ id: numericId, nombre: values.nombre, cuit: values.cuit });
        ...
        <Box >
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 6 }}>
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <ArrowbackButton />
                    <Typography variant="h1">Editar Diagrama Trabajo</Typography>
                </Box>
            </Box>
...
export default EditDiagramaTrabajo;
