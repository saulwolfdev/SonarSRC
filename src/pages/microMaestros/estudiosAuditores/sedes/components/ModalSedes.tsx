...
import { fetchSedes } from "@/services/microMaestros/SedesServices";
import { SedeFiltradoRequest } from "@/types/microMaestros/SedesTypes";
...
const ModalSedes: React.FC<ModalEstadoProps> = ({
  open,
  handleClose,
  id,
  toggleActivation,
  ...
  const buscarFuncEstId = async () => {
    ...
    const response = await fetchSedes(filtrosAplicados)   
      ...
      >
        ...
        >
          <Typography id="modal-title" variant="h1">
            {isActivated ? "Desactivar" : "Activar"} sede
          </Typography>
