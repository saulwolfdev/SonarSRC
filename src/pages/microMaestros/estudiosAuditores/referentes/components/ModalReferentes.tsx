...
import { fetchReferentes } from "@/services/microMaestros/ReferentesServices";
import { ReferenteFiltradoRequest } from "@/types/microMaestros/ReferentesTypes";
...
const ModalReferentes: React.FC<ModalEstadoProps> = ({
  open,
  handleClose,
  id,
  toggleActivation,
  ...
  const buscarFuncEstId = async () => {
    ...
    const response = await fetchReferentes(filtrosAplicados)   
      ...
      >
        ...
        >
          <Typography id="modal-title" variant="h1">
            {isActivated ? "Desactivar" : "Activar"} referente
          </Typography>
...
export default ModalReferentes;
