...
import { ClasificacionCentroFisicoDetalleResponce, ClasificacionCentroFisicoGridData } from "@/types/microMaestros/clasificacionCentrosFisicosTypes";
import { fetchClasificacionCentroFisicoById } from "@/services/microMaestros/clasificacionCentroFisicoService";
...
const ModalEstado: React.FC<ModalEstadoProps> = ({
  ...
  const buscarClasificacionId =async () => {
    const response = await fetchClasificacionCentroFisicoById(id)
    setclasificacionNombre(response.nombre)
    setCantRegistros(response.cantidadCentrosFisicos) // todo ya no lo trae mas
      ...
      >
        ...
        >
            <Typography
              ...
              component="h2"
            >
            {isActivated ? "Desactivar" : "Activar"} clasificación de centro físico
          </Typography>
