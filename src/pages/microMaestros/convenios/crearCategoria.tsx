...
import { InputsCreateUpdateCategoria } from "@/components/microMaestros/convenios/formCreateUpdate/InputCreateUpdateCategoria";
import { postCategoria } from "@/services/microMaestros/categoriasService";
...
export default function CrearClasificaionCentroFisico() {
  ...
  const {  codigoConvenioColectivo ,codigoTituloConvenioColectivo, nombreConvenioColectivo} = useCodigosStore();

  const formik = useFormik({
    initialValues: {
      nombre: "",
      estado: true,
      CodigoTituloConvenioColectivo : Number(codigoTituloConvenioColectivo)
    },
  ...
  useEffect(() =>{
    if(!codigoConvenioColectivo || !codigoTituloConvenioColectivo){
      routerPush('/microMaestros/convenios')

    }
  }, [codigoConvenioColectivo, codigoTituloConvenioColectivo])
  ...
  return (
    ...
    >
      ...
      <Box display="flex" alignItems="center">
        <Typography variant="body1" sx={{ ml:45, mt:10, fontWeight: 'bold', textAlign: "center", flexGrow: 1, color: "#707070" }}>
          Convenio:  {nombreConvenioColectivo}
        </Typography>
      </Box>

      <form onSubmit={formik.handleSubmit} style={{ width: "100%" }}>
        <InputsCreateUpdateCategoria formik={formik} />
